  ...
  while True:
    ...
    for event in pygame.event.get():
      ...
      if event.type == KEYDOWN:
        ...
        if event.key == K_1:
          while True:
            plst = str('playerstatas.MAXHP','playerstatas.HP')
            text3=font.render(plst,True,(255,255,255))
            pygame.draw.rect(screen, (0,0,0), (70,140,400,120))
            screen.blit(text3,[75,200])
            pygame.display.update()
            for event in pygame.event.get():
              if event.type == QUIT:          
                pygame.quit()
